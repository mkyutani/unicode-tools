...
def convert_to_halfwidth(text):
    ...
    return ''.join(
        chr(ord(char) - 0xFEE0) if 0xFF01 <= ord(char) <= 0xFF5E
        else ' ' if char == '　'  # Fullwidth space to regular space
        else char
        for char in text
    )
...
def normalize_text(text, form='NFC', halfwidth=False):
    ...
    normalized = unicodedata.normalize(form.upper(), text) if form.upper() in ['NFC', 'NFD', 'NFKC', 'NFKD'] else text
...
def normalize_command(form='NFC', halfwidth=False, compare=False, detailed=False, delimiter=' ', input_file=None):
    ...
    try:
        ...
        if compare:
            ...
            if detailed:
                ...
                print(result, end='')  # Don't add extra newline since content may already have it
            
    except FileNotFoundError:
        print(f"Error: File '{input_file}' not found", file=sys.stderr)
        return 1
    except UnicodeDecodeError:
        print(f"Error: Unable to decode file as UTF-8", file=sys.stderr)
        return 1
