...
Examples:
  ...
  uchr db info                        # Show database location
    ...
    search_parser.add_argument('expression', metavar='EXPR', help='Expression to search')
    
    search_by_group = search_parser.add_mutually_exclusive_group()
    search_by_group.add_argument('-b', '--block', action='store_const', dest='by', const='block', help='Search by block name')
    search_by_group.add_argument('-c', '--code', action='store_const', dest='by', const='code', help='Search by code point or range')
    search_by_group.add_argument('-x', '--char', action='store_const', dest='by', const='char', help='Search by character')
    search_by_group.add_argument('-d', '--detail', action='store_const', dest='by', const='detail', help='Search by character details')
    
    search_parser.add_argument('-s', '--strict', action='store_true', help='Match name strictly (case insensitive)')
    search_parser.add_argument('-1', '--first', action='store_true', help='Show first result only')
    ...
    # db create
    db_create_parser = db_subparsers.add_parser('create', help='Create Unicode database')
    db_create_parser.set_defaults(func=db_create_command)
    
    # db delete  
    db_delete_parser = db_subparsers.add_parser('delete', help='Delete Unicode database')
